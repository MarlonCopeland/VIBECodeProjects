...
//
// On the offline demo backend the code is logged to the console ([RESET] ...).
//
...
export default function ResetPasswordScreen() {
  ...
  const consumeLink = useCallback(
    ...
    [redeemAuthLink, status, email, setError],
  );
  ...
  const submitCode = () =>
    run(async () => {
      await verifyEmailCode(email, code.trim(), 'recovery');
      setPhase('password');
    });
  ...
  if (phase === 'done') {
    return (
      <Screen scroll center>
        ...
        <Button title="Continue to Vendor Finder" onPress={() => router.replace('/(app)/(tabs)')} />
        <View style={{ marginTop: spacing.md }}>
          <Button
            title="Sign out"
            variant="ghost"
