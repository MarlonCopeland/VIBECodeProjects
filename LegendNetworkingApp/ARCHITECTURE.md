...
This document explains how UnjadedDigital is put together and how to extend it.
...
`src/backend/types.ts` defines `Backend = { auth, profile, notifications }`.
Both `localBackend` and `supabaseBackend` implement it exactly. `backend/index.ts`
resolves one at startup from `APP_BACKEND`:
...
enabled, routes }`), so tooling, settings screens, and the `UnjadedDigital`
namespace can enumerate what's active.

## The `UnjadedDigital` namespace
...
UnjadedDigital.features.payments;          // boolean
UnjadedDigital.backend.auth.getSession();  // active backend
UnjadedDigital.modules.profile.routes;     // ['(app)/profile/edit']
```
