...
- **Web/PWA:** `react-native-web`, served by Expo's Metro bundler (PWA manifest in `app.config.js`)
- **Navigation:** `@react-navigation/native` (stack + bottom tabs)
- **Backend:** swappable — `local` (AsyncStorage, offline) or `supabase` (Postgres + Auth + Edge Functions + Realtime)
- **Auth:** Supabase Auth (email/password + Google + Facebook OAuth via `expo-auth-session`); local demo auth offline
- **Payments:** Stripe Checkout via Supabase Edge Functions, behind a pluggable `PaymentProvider` interface
- **Location:** `expo-location` with a mock fallback
- **Notifications:** typed vendor→follower alerts; local + Expo Push (remote fan-out from an Edge Function)
- **Config:** env-injected via `app.config.js` → `expo-constants` (see `.env.example`)
- **Language:** JavaScript (ES modules); Edge Functions in TypeScript/Deno

### New since auth/Supabase/subscriptions work
- `src/config/tiers.js` — single source of truth for notification types, weekly quotas, follower gate, and subscription tiers (data-driven).
- `src/config/env.js` + `app.config.js` — runtime config + `APP_BACKEND` selection.
- `src/services/backend/supabaseBackend.js` (+ `supabaseClient.js`) — replaces the old Firebase stub.
- `src/services/payments/*` — `PaymentProvider`, `StripePaymentProvider`, `LocalPaymentProvider`, selector.
- `src/screens/SubscriptionScreen.js` — tier management UI.
- `supabase/schema.sql` + `supabase/functions/*` — DB schema/RLS + Edge Functions.
- Setup docs: `SUPABASE_SETUP.md`, `DEPLOYMENT.md`.
...
### Email verification (REQUIRED for all sign-up methods)
- **No app feature is accessible until the account's email is verified.**
  `AuthContext.emailVerified` gates everything; `App.js` renders
  `VerifyEmailScreen` (and nothing else) for an authenticated-but-unverified
  user, and `can()` returns `false` for any action while unverified.
- **Email/password:** new accounts start **unverified**.
  - Supabase: a confirmation email is sent (enable *Confirm email* in the
    dashboard). The user clicks the link; the app detects the new status via
    `refreshSession()` (polled every 5s on the verify screen) and unlocks.
  - Local/demo backend: a 6-digit code is generated and logged to the console
    (`[VERIFY] …`); enter it on `VerifyEmailScreen`. `confirmVerification(code)`
    flips the flag.
- **Google / Facebook OAuth:** providers return a verified email, so these
  accounts pass the gate immediately.
- `VerifyEmailScreen` supports **resend** (30s cooldown) and **sign out**.
- Vendors: the paired `vendor` record is created at sign-up when possible, and
  otherwise **backfilled after verification** via `ensureVendorRecord()` (covers
  Supabase's deferred-session case when email confirmation is on).
