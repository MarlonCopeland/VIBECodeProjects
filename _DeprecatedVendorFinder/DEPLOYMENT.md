...
2. **Add Apple IAP** before public iOS release. The payment layer is pluggable:
   implement `AppleIapProvider extends PaymentProvider` in
   `src/services/payments/`, register it in `payments/index.js` for `ios`, and
   keep Stripe for web. No UI/business-logic changes required.
