 ...
 * the given user (so vendors' notifications can fan out via Expo Push from the
 * Supabase Edge Function). Safe to call repeatedly. No-op on web / simulators.
 */
export async function registerForPushNotifications(userId) {
  try {
    ...
    if (token && userId) {
      await backend.registerPushToken(userId, token);
    }
    return token;
  } catch (e) {
